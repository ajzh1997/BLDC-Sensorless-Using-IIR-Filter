...
volatile unsigned int Lock1Demand = 50 ;			// % demand for first lock position
volatile unsigned int Lock2Demand = 50;			// % demand for second lock position
volatile unsigned int Lock1Duration = 100;		// Lock duration in 1ms increments
volatile unsigned int Lock2Duration = 100;		// Lock duration in 1ms increments
volatile unsigned int RampStartDemand = 25;		// % demand at the beginning of the open-loop starting ramp
volatile unsigned int RampEndDemand = 55;		// % demand at the end of the open-loop starting ramp
volatile unsigned int RampDuration = 1000;		// Ramp duration in 1ms increments
volatile unsigned int RampStartSpeed = 80;		// Speed (in RPM) at the beginning of the open-loop starting ramp
volatile unsigned int RampEndSpeed = 1000;		// Speed (in RPM) at the end of the open-loop starting ramp
...
volatile unsigned int BlankingCountInit = 2;	// The number of ADC samples "ignored" after commutating the motor windings
volatile unsigned int SpeedControl_P = 120;		// The P term for the PI speed control loop
volatile unsigned int SpeedControl_I = 0;		// The I term for the PI speed control loop
volatile unsigned int SpeedControl_D = 0;		// The I term for the PI speed control loop
volatile unsigned int PhaseAdvanceStart = 2000;	// The RPM at which point phase advance begins to be added
...
volatile unsigned int InitialDemand = 55;		// % demand immediately after the starting ramp is completed.
volatile unsigned int NoOfMotorPoles = 12;		// Number of motor poles (not pole-pairs)
volatile unsigned int CrossOverERPS = 1000; 		// low speed to high speed (and vice versa) crossover speed in electrical revs per second
volatile unsigned int NumOfTimer1TimeOuts = 15;  // Defines the number of Timer 1 timeouts that will sequentially occur before the motor shuts down
